...
namespace {
    ...
    PythonLayout resolve_layout() {
        ...
        if (!stdlib.has_value()) {
            throw std::runtime_error("bundled Python stdlib was not found under " + (layout.sdk_root / "lib").string());
        }
        ...
        if (!fs::is_directory(layout.site_packages, ec)) {
            throw std::runtime_error("bundled Python site-packages is missing: " + layout.site_packages.string());
        }
        return layout;
    }
        ...
        while (index < argc) {
            ...
            if (argument == "--termin-overlay") {
                ...
                if (ec || !fs::is_regular_file(manifest, ec)) {
                    throw std::runtime_error("Python overlay manifest is missing: " + manifest.string());
                }
    ...
    int run_python(const PythonLayout& layout, const LauncherOptions& options) {
        ...
        const termin::python_host::InitResult initialized = termin::python_host::initialize(config);
        if (!initialized.ok) {
        ...
        for (auto site = options.extra_sites.rbegin(); site != options.extra_sites.rend(); ++site) {
            ...
            if (path == nullptr || PyList_Insert(sys_path, 0, path) != 0) {
                Py_XDECREF(path);
                PyErr_Print();
                std::cerr << "termin_python: failed to add extra Python site: " << site->string() << std::endl;
                return 1;
            }
            Py_DECREF(path);
        }
        if (options.overlay_manifest.has_value()) {
            ...
            PyObject* result = PyObject_CallMethod(
                overlay_module, "activate_overlay", "s", options.overlay_manifest->string().c_str());
            Py_DECREF(overlay_module);
            if (result == nullptr) {
                PyErr_Print();
                std::cerr << "termin_python: failed to activate Python overlay: " << options.overlay_manifest->string()
                          << std::endl;
                return 1;
            }
            Py_DECREF(result);
