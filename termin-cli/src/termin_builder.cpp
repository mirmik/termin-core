...
namespace {
    ...
    void print_help() {
        std::cout
            ...
            << "  termin_builder capabilities <name> [--project <dir>] [--profiles <file>] [toolchain options] "
               "[--json]\n"
    ...
    void print_usage_error() {
        std::cerr << "Usage: termin_builder profiles|profile <name>|capabilities <name>|build <name> [options]\n"
    ...
    ParsedArgs parse_args(int argc, char** argv) {
        ...
        if (parsed.command != "profiles" && parsed.command != "profile" && parsed.command != "capabilities" &&
            parsed.command != "build" && parsed.command != "resolve") {
            throw std::runtime_error("unknown command: " + parsed.command);
        }
        const bool accepts_toolchain_options = parsed.command == "build" || parsed.command == "capabilities";

        int i = 2;
        if (parsed.command == "profile" || parsed.command == "capabilities" || parsed.command == "build" ||
    ...
    profile_backend_command(const ParsedArgs& args, const fs::path& project_root, const fs::path& profiles_path) {
        ...
        if (args.command == "build" || args.command == "capabilities") {
            const auto append_path_option = [&command](const char* name, const fs::path& value) {
                if (!value.empty()) {
            ...
            append_path_option("--quest-openxr-build-script", args.options.quest_openxr_build_script);
    ...
    int run_profile_backend(const ParsedArgs& args) {
        ...
        const std::vector<std::string> command = profile_backend_command(args, project_root, profiles_path);
