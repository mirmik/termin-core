...
namespace {
    ...
    void print_usage_error() {
        std::cerr << "Usage: termin_runner run <profile> [options]\n"
    ...
    ParsedArgs parse_args(int argc, char** argv) {
        ...
        for (; i < argc; ++i) {
            ...
            if (arg == "--project") {
                ...
                parsed.options.player_args.emplace_back(arg);
            } else if (arg == "--mcp-host" || arg == "--mcp-port" || arg == "--mcp-token" ||
                       arg == "--mcp-session-file") {
                parsed.options.player_args.emplace_back(arg);
                parsed.options.player_args.emplace_back(take_value(argc, argv, i, arg));
            } else if (arg == "--width" || arg == "--height" || arg == "--title" || arg == "-W" || arg == "-H" ||
                       arg == "-t") {
                parsed.options.player_args.emplace_back(arg);
                parsed.options.player_args.emplace_back(take_value(argc, argv, i, arg));
            } else {
        ...
        if (parsed.command == "run" && parsed.options.mode != "build" && parsed.options.mode != "project") {
    ...
    std::optional<std::string> resolve_scene_candidate(const fs::path& project_root,
    ...
    std::optional<std::string> read_last_scene_from_project_state(const fs::path& project_root) {
        ...
        try {
            ...
            if (!state.is_dict()) {
                std::cerr << "termin_runner: editor state root is not an object: " << state_path << "\n";
                return std::nullopt;
            }
            ...
            if (!last_scene->is_string()) {
                std::cerr << "termin_runner: editor state last_scene is not a string: " << state_path << "\n";
                return std::nullopt;
            }
            return last_scene->as_string();
        } catch (const std::exception& exc) {
            std::cerr << "termin_runner: failed to read editor state " << state_path << ": " << exc.what() << "\n";
            return std::nullopt;
        }
    }
    ...
    std::optional<std::string> find_first_project_scene(const fs::path& project_root) {
        ...
        for (; !ec && it != end; it.increment(ec)) {
            ...
            if (entry.is_directory(ec)) {
                ...
                if (name == "project_settings" || name.rfind(".", 0) == 0 || name.rfind("__", 0) == 0) {
        ...
        if (ec) {
            std::cerr << "termin_runner: failed to scan project scenes: " << ec.message() << "\n";
        }
        if (scenes.empty()) {
            return std::nullopt;
        }
    ...
    std::string resolve_play_scene(const fs::path& project_root, const ParsedArgs& args) {
        if (args.options.scene_override.has_value()) {
            return *resolve_scene_candidate(project_root, *args.options.scene_override, "--scene", true);
        }
        ...
        if (std::optional<std::string> last_scene = read_last_scene_from_project_state(project_root)) {
            if (std::optional<std::string> resolved =
                    resolve_scene_candidate(project_root, *last_scene, "last_scene", false)) {
                return *resolved;
            }
        }
    ...
    std::string required_request_string(const nos::trent& object, const char* key) {
        ...
        if (value == nullptr || !value->is_string() || value->as_string().empty()) {
            throw std::runtime_error(std::string("resolved build request field must be a non-empty string: ") + key);
        }
        return value->as_string();
    }

    BuildProfile resolve_profile_request(const fs::path& project_root,
                                         const fs::path& profiles_path,
                                         const std::string& profile_name) {
        configure_python_backend_environment();
        const auto nonce = std::chrono::high_resolution_clock::now().time_since_epoch().count();
        const fs::path request_path =
            fs::temp_directory_path() / ("termin-profile-request-" + std::to_string(nonce) + ".json");
        ...
        if (resolve_code != 0) {
            ...
            throw std::runtime_error("failed to resolve build profile '" + profile_name +
    ...
    std::optional<fs::path> read_bundle_launcher_path(const fs::path& output_dir, const fs::path& app_manifest_path) {
        std::error_code ec;
        ...
        if (!runtime->is_dict()) {
            throw std::runtime_error("app manifest field 'runtime' must be a JSON object: " +
        ...
        if (!launcher->is_string()) {
            throw std::runtime_error("app manifest field 'runtime.launcher' must be a string: " +
                                     app_manifest_path.string());
        }
    ...
    std::vector<fs::path> packaged_launcher_candidates(const fs::path& output_dir, const fs::path& app_manifest_path) {
    ...
    std::optional<fs::path> resolve_packaged_launcher(const fs::path& output_dir, const fs::path& app_manifest_path) {
        std::error_code ec;
    ...
    int maybe_build_profile(const ParsedArgs& args,
        ...
        if (!args.options.rebuild && !args.options.build_if_missing) {
            std::cerr << "termin_runner: build output does not exist for profile '" << args.profile_name << "'.\n"
                      << "Run 'termin build " << args.profile_name << " --project " << project_root.string()
    ...
    int command_run(const ParsedArgs& args) {
        ...
        if (profile.target != "desktop" && args.options.mode == "build") {
            std::cerr << "termin_runner: unsupported build run target '" << profile.target << "'.\n";
            return 3;
        }
        ...
        if (args.options.mode == "build") {
            ...
            int build_code = maybe_build_profile(args, project_root, profiles_path, !has_desktop_bundle);
            if (build_code != 0) {
                return build_code;
            }
            ...
            if (has_bundle_after_build) {
                ...
                if (!player_path.has_value()) {
                    std::cerr << "termin_runner: packaged desktop bundle player does not exist: " << app_manifest_path
                              << "\n"
            ...
            if (!has_bundle_after_build) {
                std::cerr << "termin_runner: packaged desktop bundle does not exist: " << app_manifest_path << "\n"
                          << "Run 'termin build " << args.profile_name << " --project " << project_root.string()
