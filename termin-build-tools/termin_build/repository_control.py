...
_PYTEST_DURATION_CACHE_SCHEMA = 1
_PYTEST_DURATION_SMOOTHING = 0.5
...
def _run_timed_pytest_command(
    ...
    *,
    ...
    returncode, output = _run_pytest_command(
        command,
        repo_root,
        environment,
...
def _load_pytest_duration_cache(path: Path) -> dict[str, dict[str, float]]:
    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(raw, dict) or raw.get("schema") != _PYTEST_DURATION_CACHE_SCHEMA:
        ...
        print(
            f"WARNING: ignoring invalid pytest duration cache {path}: {exc}",
            file=sys.stderr,
        )
        return {}


def _write_pytest_duration_cache(
    path: Path, profiles: dict[str, dict[str, float]]
) -> None:
    temporary_path = path.with_name(f".{path.name}.{uuid.uuid4().hex}.tmp")
    payload = {
        "schema": _PYTEST_DURATION_CACHE_SCHEMA,
        "profiles": profiles,
    }
    try:
        ...
        print(
            f"WARNING: failed to update pytest duration cache {path}: {exc}",
            file=sys.stderr,
        )
        try:
...
def _updated_pytest_durations(
    previous: dict[str, float], measured: dict[str, float]
) -> dict[str, float]:
    updated = dict(previous)
    for suite_id, duration in measured.items():
        old_duration = previous.get(suite_id)
        if old_duration is None:
            updated[suite_id] = duration
        else:
            updated[suite_id] = (
                old_duration * (1.0 - _PYTEST_DURATION_SMOOTHING)
                + duration * _PYTEST_DURATION_SMOOTHING
            )
    return updated


def _print_pytest_suite_header(suite_id: str) -> None:
    print("")
    print("----------------------------------------")
    print(f"  {suite_id}")
    print("----------------------------------------")
...
def _pytest_worker_environment(
    base_environment: dict[str, str], suite_temp: Path
) -> dict[str, str]:
...
def _report_pytest_suite_result(
    suite_id: str, returncode: int, output: str, *, print_output: bool
) -> bool:
    if print_output:
        _print_pytest_suite_header(suite_id)
...
def run_pytest_plan(
    ...
    print(
        ...
        f"{sum(suite['id'] in historical_durations for suite in pytest_suites)}"
    ...
    for manifest_index, suite in enumerate(pytest_suites):
        suite_id = suite["id"]
        ...
        suite_runs.append(
            (
                suite_id,
                command,
                _pytest_worker_environment(base_environment, suite_temp),
                manifest_index,
            )
        )
    suite_runs.sort(
        key=lambda run: (
            -historical_durations.get(run[0], -1.0),
            run[3],
        )
    )

    failed_suite_ids = set()
    measured_durations = {}
    if jobs == 1:
        for suite_id, command, environment, _ in suite_runs:
            _print_pytest_suite_header(suite_id)
            sys.stdout.flush()
            returncode, output, duration = _run_timed_pytest_command(
                command,
                repo_root,
                environment,
                stream_output=True,
            )
            measured_durations[suite_id] = duration
            if _report_pytest_suite_result(
                suite_id, returncode, output, print_output=False
            ):
                failed_suite_ids.add(suite_id)
    else:
        with concurrent.futures.ThreadPoolExecutor(max_workers=jobs) as executor:
            future_suites = {
                executor.submit(
                    _run_timed_pytest_command,
                    command,
                    repo_root,
                    environment,
                    stream_output=False,
            ...
            for future in concurrent.futures.as_completed(future_suites):
                suite_id = future_suites[future]
                returncode, output, duration = future.result()
                measured_durations[suite_id] = duration
                if _report_pytest_suite_result(
                    suite_id, returncode, output, print_output=True
                ):
                    failed_suite_ids.add(suite_id)

    duration_profiles[duration_profile_key] = _updated_pytest_durations(
        historical_durations, measured_durations
    )
    _write_pytest_duration_cache(duration_cache_path, duration_profiles)

    failures = [
        suite["id"] for suite in pytest_suites if suite["id"] in failed_suite_ids
    ]
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_parser.add_argument(
        ...
        default=None,
        help=(
            "Maximum concurrent pytest suites "
            "(default: TERMIN_PYTEST_JOBS or 1)."
        ),
    )
    ...
    try:
        ...
        if args.command == "run":
            ...
            if pytest_jobs is None:
                raw_pytest_jobs = os.environ.get("TERMIN_PYTEST_JOBS", "1")
                try:
                    pytest_jobs = int(raw_pytest_jobs)
                except ValueError as exc:
                    raise ManifestError(
                        "TERMIN_PYTEST_JOBS must be an integer"
                    ) from exc
            return _cmd_run(
                repo_root,
                args.profile,
