...
static tc_dispatch_callback_result add_value(void* user_data) {
    ...
    *payload->value += payload->increment;
    ...
    if (payload->post_nested) {
        ...
        assert(tc_dispatcher_post(payload->dispatcher, add_value, dispose_payload, nested, NULL));
    }

    if (payload->recurse) {
        const tc_dispatch_stats recursive = tc_dispatcher_drain(payload->dispatcher, TC_DISPATCH_ALL);
...
static test_payload* make_payload(tc_dispatcher* dispatcher, int* value, int increment, int* dispose_count) {
...
int main(void) {
    ...
    assert(tc_dispatcher_post(
        dispatcher, add_value, dispose_payload, make_payload(dispatcher, &value, 1, &dispose_count), &first));
    assert(tc_dispatcher_post(
        dispatcher, add_value, dispose_payload, make_payload(dispatcher, &value, 10, &dispose_count), &second));
    ...
    test_payload* nesting = make_payload(dispatcher, &value, 5, &dispose_count);
    nesting->post_nested = true;
    nesting->recurse = true;
    assert(tc_dispatcher_post(dispatcher, add_value, dispose_payload, nesting, NULL));
    ...
    assert(tc_dispatcher_post(dispatcher, fail_callback, NULL, NULL, NULL));
    stats = tc_dispatcher_drain(dispatcher, TC_DISPATCH_ALL);
    assert(stats.executed == 1);
    assert(stats.failed == 1);

    test_payload* discarded = make_payload(dispatcher, &value, 1000, &dispose_count);
    assert(tc_dispatcher_post(dispatcher, add_value, dispose_payload, discarded, NULL));
    ...
    test_payload* rejected = make_payload(dispatcher, &value, 1, &dispose_count);
    assert(!tc_dispatcher_post(dispatcher, add_value, dispose_payload, rejected, NULL));
