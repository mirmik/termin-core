...
struct tc_dispatcher {
    ...
    tc_dispatch_ticket allocate_locked(tc_dispatch_callback callback, tc_dispatch_dispose dispose, void* user_data) {
...
bool tc_dispatcher_post(tc_dispatcher* dispatcher,
    ...
    try {
        ...
        const tc_dispatch_ticket ticket = dispatcher->allocate_locked(callback, dispose, user_data);
        if (out_ticket) {
            *out_ticket = ticket;
        }
        return true;
    ...
    try {
        ...
        std::vector<DispatchWork> batch;
        {
            ...
            const size_t batch_limit =
                limit == TC_DISPATCH_ALL ? dispatcher->queued_count : std::min(limit, dispatcher->queued_count);
            ...
            while (!dispatcher->queue.empty() && batch.size() < batch_limit) {
                ...
                --dispatcher->queued_count;
                batch.push_back(DispatchWork{ticket, slot->callback, slot->dispose, slot->user_data});
            }
        }
        ...
        for (const DispatchWork& work : batch) {
...

            {
                ...
                if (!slot || slot->state != SlotState::Executing) {
                    log_dispatch_error("callback completion", "executing slot lost its lifetime state");
...
namespace termin {
    ...
    DeferredCall Dispatcher::defer(std::function<void()> callback) {
        ...
        DeferredCall call = post(&invoke_cpp_callback, &dispose_cpp_callback, payload);
        if (!call) {
            delete payload;
        }
        return call;
    ...
    DeferredCall Dispatcher::post(tc_dispatch_callback callback, tc_dispatch_dispose dispose, void* user_data) {
        if (!state_) {
            return {};
        }
        tc_dispatch_ticket ticket = tc_dispatch_ticket_invalid();
        if (!tc_dispatcher_post(state_->native, callback, dispose, user_data, &ticket)) {
            return {};
        }
