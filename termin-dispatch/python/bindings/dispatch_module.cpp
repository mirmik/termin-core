...
namespace {
    ...
    struct PythonCallback {
        explicit PythonCallback(PyObject* object)
            : object(object) {
            Py_INCREF(object);
        }
    ...
    void dispose_python_callback(void* user_data) {
        ...
        if (Py_IsInitialized()) {
            ...
            tc_log_error("[termin-dispatch/python] Python finalized before callback disposal");
        }
        delete payload;
    }
    ...
    public:
        termin::DeferredCall defer(nb::object callback) {
            ...
            std::unique_ptr<PythonCallback> payload(new PythonCallback(callback.ptr()));
            termin::DeferredCall call =
                dispatcher_.post(&invoke_python_callback, &dispose_python_callback, payload.get());
            if (!call) {
        ...
        tc_dispatch_stats run_pending(nb::object limit) {
            const size_t native_limit = limit.is_none() ? TC_DISPATCH_ALL : nb::cast<size_t>(limit);
        ...
        size_t discard_pending() {
            return dispatcher_.discard_pending();
        }
        bool is_open() const {
            return dispatcher_.is_open();
        }
        size_t pending_count() const {
            return dispatcher_.pending_count();
        }
...
NB_MODULE(_dispatch_native, module) {
    module.doc() = "Caller-driven deferred dispatcher backed by the Termin C ABI";
    ...
    nb::class_<termin::DeferredCall>(module, "DeferredCall")
        ...
        .def_prop_ro("valid", [](const termin::DeferredCall& call) { return static_cast<bool>(call); });

    nb::class_<PythonDispatcher>(module, "Dispatcher")
        .def(nb::init<>())
        .def("defer", &PythonDispatcher::defer, "callback"_a)
        .def("run_pending", &PythonDispatcher::run_pending, "limit"_a = nb::none())
