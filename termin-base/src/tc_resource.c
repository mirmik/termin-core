...
void tc_resource_set_loader(tc_resource_loader_fn callback, void* user_data) {
...
bool tc_resource_request_load(const char* uuid) {
    ...
    if (!g_resource_loader) {
        tc_log_warn("tc_resource_request_load: no process resource loader is installed for '%s'", uuid);
        return false;
    }
    if (!g_resource_loader(uuid, g_resource_loader_user_data)) {
        tc_log_error("tc_resource_request_load: process resource loader failed for '%s'", uuid);
        return false;
    }
    return true;
}
