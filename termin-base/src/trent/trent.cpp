...
nos::trent::wrong_path::wrong_path(const nos::trent_path& path)
    : path(path) {
...
nos::trent::wrong_type::wrong_type(const trent_path& path, type t, type rt)
    : path(path),
      t(t) {
    str = std::string("trent:wrong_type:") + std::string(" path:") + path.to_string() + std::string(" request:") +
          nos::typestr(t) + std::string(" realtype:") + nos::typestr(rt);
}
...
nos::trent::wrong_index::wrong_index(const trent_path& path, type t)
    : path(path),
      t(t) {
    str = std::string("trent:wrong_index: path: ") + path.to_string() + std::string(" index: ") + nos::typestr(t);
}
...
nos::trent::trent(const trent& other)
    : m_type(other.m_type) {
    switch (m_type) {
...
nos::trent::trent(trent&& other) noexcept
    : m_type(other.m_type) {
    switch (m_type) {
    case trent::type::string:
...
nos::trent& nos::trent::at(int i) {
    if (m_type != type::list)
        throw std::runtime_error(std::string("trent: is not a list: ") + nos::typestr(m_type));
    if (m_arr.size() <= (unsigned int)i)
        throw std::runtime_error(std::string("trent:wrong_index: ") + std::to_string(i));
    return m_arr[i];
}
...
const nos::trent& nos::trent::at(const std::string& key) const {
    if (m_type != type::dict)
        throw std::runtime_error(std::string("trent: is not a dict: ") + nos::typestr(m_type));
    return m_dct.at(key);
}
...
const nos::trent::string_type& nos::trent::get_as_string_ex(const trent_path& path) const {
    const trent& tr = get_except(path);
...
nos::trent::numer_type nos::trent::get_as_numer_def(const trent_path& path, numer_type def) const {
    const trent* tr = get(path);
...
const nos::trent::string_type& nos::trent::get_as_string_def(const trent_path& path, const std::string& def) const {
    const trent* tr = get(path);
...
const nos::trent::string_type& nos::trent::as_string() const {
    if (!is_string())
        throw std::runtime_error(std::string("trent: is not a string: ") + nos::typestr(m_type));
    return m_str;
}

nos::expected<nos::trent::string_type&, nos::errstring> nos::trent::as_string_critical() {
    if (!is_string())
        return errstring("is't string");
    return m_str;
}

nos::expected<const nos::trent::string_type&, nos::errstring> nos::trent::as_string_critical() const {
    if (!is_string())
        return errstring("is't string");
    return m_str;
}
...
const nos::trent::string_type& nos::trent::as_string_default(const string_type& def) const {
    if (!is_string())
        return def;
    return m_str;
}
...
nos::expected<nos::trent::dict_type&, nos::errstring> nos::trent::as_dict_critical() {
    if (!is_dict())
        return errstring("is't list");
    return m_dct;
}

nos::expected<const nos::trent::dict_type&, nos::errstring> nos::trent::as_dict_critical() const {
    if (!is_dict())
        return errstring("is't list");
    return m_dct;
}
...
nos::expected<nos::trent::list_type&, nos::errstring> nos::trent::as_list_critical() {
    if (!is_list())
        return errstring("is't list");
    return m_arr;
}

nos::expected<const nos::trent::list_type&, nos::errstring> nos::trent::as_list_critical() const {
    if (!is_list())
        return errstring("is't list");
    return m_arr;
}
...
nos::expected<nos::trent::numer_type, nos::errstring> nos::trent::as_numer_critical() const {
    if (is_bool())
        return (int)m_bool;
