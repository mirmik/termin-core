...
namespace nos {
    namespace yaml {
        parse_error::parse_error(size_t line, size_t column, const std::string& msg)
            : std::runtime_error([&]() {
        ...
        namespace detail {
            ...
            inline size_t offset_for_indent(const std::string& text, int indent) {
                int col = 0;
                size_t i = 0;
            ...
            static std::string remove_numeric_separators(const std::string& text) {
                std::string out;
            ...
            static std::string parse_single_quoted(const std::string& text, size_t line, size_t column) {
                if (text.size() < 2 || text.back() != '\'')
                    throw nos::yaml::parse_error(line, column, "unterminated single-quoted string");

                std::string result;
                for (size_t i = 1; i < text.size() - 1; ++i) {
                    char ch = text[i];
                    if (ch == '\'' && i + 1 < text.size() - 1 && text[i + 1] == '\'') {
                        result.push_back('\'');
                        ++i;
                    } else {
                        result.push_back(ch);
            ...
            static std::string parse_double_quoted(const std::string& text, size_t line, size_t column) {
                if (text.size() < 2 || text.back() != '"')
                    throw nos::yaml::parse_error(line, column, "unterminated double-quoted string");
                ...
                for (size_t i = 1; i < text.size() - 1; ++i) {
                    ...
                    if (ch == '\\') {
                        if (i + 1 >= text.size() - 1)
                            throw nos::yaml::parse_error(line, column + i, "bad escape sequence");
                        ...
                        case 'u': {
                            if (i + 4 >= text.size() - 1)
                                throw nos::yaml::parse_error(line, column + i, "incomplete unicode escape");
                            std::string hex = text.substr(i + 1, 4);
                            uint32_t code = hex_to_int(hex);
                            if (code == std::numeric_limits<uint32_t>::max())
                                throw nos::yaml::parse_error(line, column + i, "invalid unicode escape");
                            if (code <= 0x7F) {
                                result.push_back(static_cast<char>(code & 0x7F));
                            } else if (code <= 0x7FF) {
                        ...
                        default:
                            throw nos::yaml::parse_error(line, column + i, "invalid escape sequence");
                            break;
                        }
                        continue;
                    }
            ...
            static nos::trent parse_scalar_text(const std::string& text, size_t line, size_t column) {
                ...
                if (lower == ".inf" || lower == "+.inf")
                    return nos::trent(std::numeric_limits<long double>::infinity());
                if (lower == "-.inf")
                    return nos::trent(-std::numeric_limits<long double>::infinity());
                if (lower == ".nan")
                    return nos::trent(std::numeric_limits<long double>::quiet_NaN());

                std::string numeric = remove_numeric_separators(trimmed);

                char* endptr = nullptr;
                double value = std::strtod(numeric.c_str(), &endptr);
                if (endptr && *endptr == '\0' && endptr != numeric.c_str()) {
            ...
            class parser {
                ...
                static size_t find_block_indicator(const std::string& text, size_t start_pos) {
                    bool in_single = false;
                    bool in_double = false;
                ...
                static int compute_block_indent(const std::string& text, size_t indicator_pos, int indent) {
                ...
                void add_line(std::string line_text, size_t line_no) {
                    size_t pos = 0;
                    int indent = 0;
                    while (pos < line_text.size() && (line_text[pos] == ' ' || line_text[pos] == '\t')) {
                    ...
                    std::string no_comment = is_block_content ? line_text : strip_comment(line_text);
                    std::string trimmed = trim(no_comment);

                    lines.push_back({indent, line_text, no_comment, trimmed, line_no});
                    ...
                    if (!in_block_scalar) {
                        ...
                        if (colon != std::string::npos) {
                            value_pos = no_comment.find_first_not_of(" \t", colon + 1);
                        } else if (non_ws != std::string::npos && no_comment[non_ws] == '-') {
                            value_pos = no_comment.find_first_not_of(" \t", non_ws + 1);
                        } else {
                            value_pos = non_ws;
                        }

                        if (value_pos != std::string::npos) {
                            size_t indicator_pos = find_block_indicator(no_comment, value_pos);
                            if (indicator_pos != std::string::npos) {
                                in_block_scalar = true;
                                block_indent = compute_block_indent(no_comment, indicator_pos, indent);
                            }
                        }
                    }
                }
                ...
                bool is_sequence_line(size_t idx, int indent) const {
                    ...
                    return ln.no_comment[pos] == '-' &&
                           (pos + 1 == ln.no_comment.size() ||
                            std::isspace(static_cast<unsigned char>(ln.no_comment[pos + 1])));
                }
                ...
                bool is_mapping_line(size_t idx, int indent) const {
                    ...
                    if (ln.indent != indent)
                        return false;
                    return find_unescaped_colon(ln.no_comment) != std::string::npos;
                }
                ...
                nos::trent parse_block_scalar(const line& ln, size_t indicator_pos, size_t line_idx) {
                    ...
                    while (idx < lines.size()) {
                        ...
                        if (cur.indent < content_indent && !cur.trimmed.empty())
                            break;
                        if (cur.indent < content_indent && cur.trimmed.empty()) {
                        ...
                        size_t offset = offset_for_indent(cur.raw, content_indent);
                ...
                class flow_parser {
                    ...
                    nos::trent parse_plain_scalar() {
                        ...
                        while (!eof()) {
                            ...
                            if (c == ':') {
                                size_t next = pos + 1;
                                if (next < src.size() && !std::isspace(static_cast<unsigned char>(src[next]))) {
                                    // part of scalar
                                } else {
                                    break;
                                }
                    ...
                    nos::trent parse_double() {
                        ...
                        return parse_double_quoted(token, start_line, start_col);
                    }
                    ...
                    nos::trent parse_single() {
                        ...
                        if (token.size() < 2 || token.back() != '\'')
                            fail("unterminated string");
                        return parse_single_quoted(token, start_line, start_col);
                    }
                ...
                public:
                    flow_parser(const std::string& s, size_t base_line, size_t base_col)
                        : src(s),
                          pos(0),
                          line(base_line),
                          column(base_col),
                ...
                std::string gather_flow_text(size_t start_idx, size_t start_pos) {
                    ...
                    while (idx < lines.size()) {
                        ...
                        for (; pos < ln.no_comment.size(); ++pos) {
                            ...
                            if (!in_single && !in_double) {
                                if (ch == '[')
                                    ...
                                    if (stack.empty() || stack.back() != ch)
                                        throw nos::yaml::parse_error(ln.number,
                                                                     compute_column(ln.no_comment, pos + 1),
                                                                     "unmatched closing bracket");
                                    stack.pop_back();
                                    if (stack.empty()) {
                                        std::string tail = trim(ln.no_comment.substr(pos + 1));
                                        if (!tail.empty())
                                            throw nos::yaml::parse_error(ln.number,
                                                                         compute_column(ln.no_comment, pos + 2),
                            ...
                            if (ch == '"' && !in_single)
                                ...
                                if (in_single && pos + 1 < ln.no_comment.size() && ln.no_comment[pos + 1] == '\'') {
                                    buffer.push_back(ln.no_comment[pos + 1]);
                                    ++pos;
                                } else {
                                    in_single = !in_single;
                                }
                            } else if (ch == '\\' && in_double && pos + 1 < ln.no_comment.size()) {
                                buffer.push_back(ln.no_comment[pos + 1]);
                                ++pos;
                            }
                        }
                    ...
                    throw nos::yaml::parse_error(lines[start_idx].number,
                                                 compute_column(lines[start_idx].no_comment, start_pos + 1),
                                                 "unterminated flow collection");
                }

                nos::trent parse_flow_collection(const line& ln, size_t value_pos, size_t start_idx) {
                    size_t start_column = compute_column(ln.no_comment, value_pos + 1);
                    if (value_pos >= ln.no_comment.size())
                        throw nos::yaml::parse_error(ln.number, start_column, "invalid flow start");
                    char opener = ln.no_comment[value_pos];
                    if (opener != '[' && opener != '{')
                        throw nos::yaml::parse_error(
                            ln.number, start_column, "flow collection must start with '[' or '{'");
                    std::string flow = gather_flow_text(start_idx, value_pos);
                ...
                nos::trent parse_value(const line& ln, size_t value_pos, int indent, size_t line_idx) {
                    std::string value_text = trim(ln.no_comment.substr(value_pos));
                    size_t column = compute_column(ln.no_comment, value_pos + 1);
                ...
                nos::trent parse_block(int indent) {
                    ...
                    if (lines[index].indent < indent)
                        throw nos::yaml::parse_error(lines[index].number, 1, "invalid indentation");
                    ...
                    if (!ln.trimmed.empty() && (ln.trimmed[0] == '|' || ln.trimmed[0] == '>')) {
                        return parse_block_scalar(ln, ln.no_comment.find_first_not_of(" \t"), current_index);
                    }

                    if (!ln.trimmed.empty() && (ln.trimmed[0] == '[' || ln.trimmed[0] == '{')) {
                        size_t pos = ln.no_comment.find_first_not_of(" \t");
                        return parse_flow_collection(ln, pos == std::string::npos ? 0 : pos, current_index);
                    }

                    return parse_scalar_text(ln.trimmed, ln.number, compute_column(ln.no_comment, 1));
                }
                ...
                nos::trent parse_sequence(int indent) {
                    ...
                    while (index < lines.size()) {
                        ...
                        size_t dash_pos = ln.no_comment.find_first_not_of(" \t");
                        size_t value_pos = ln.no_comment.find_first_not_of(" \t", dash_pos + 1);
                        ...
                        if (!rest.empty()) {
                            ...
                            if (colon != std::string::npos) {
                                ...
                                size_t value_pos_in_line =
                                    ln.no_comment.find_first_not_of(" \t", value_pos + colon + 1);
                                std::string value_text = trim(rest.substr(colon + 1));
                                if (key.empty())
                                    throw nos::yaml::parse_error(ln.number,
                                                                 compute_column(ln.no_comment, value_pos + colon + 1),
                                                                 "empty key in sequence mapping");
                                if (value_text.empty() || value_pos_in_line == std::string::npos) {
                                    element[key] = nos::trent::nil();
                                } else {
                                    element[key] = parse_value(ln, value_pos_in_line, indent, current_index);
                                }
                                element_initialized = true;
                            } else {
                                element = parse_value(ln, value_pos, indent, current_index);
                                element_initialized = true;
                            }
                        }
                ...
                nos::trent parse_mapping(int indent) {
                    ...
                    while (index < lines.size()) {
                        ...
                        size_t value_pos = ln.no_comment.find_first_not_of(" \t", colon + 1);
                        std::string value_text =
                            value_pos == std::string::npos ? std::string() : trim(ln.no_comment.substr(colon + 1));

                        size_t current_index = index;

                        if (key.empty())
                            throw nos::yaml::parse_error(
                                ln.number, compute_column(ln.no_comment, 1), "empty mapping key");

                        if (!value_text.empty()) {
                            obj[key] = parse_value(ln, value_pos, indent, current_index);
                            if (index < current_index + 1)
                                index = current_index + 1;
                            continue;
                        }
            ...
            public:
                ...
                nos::trent parse() {
                    ...
                    skip_empty_lines();
                    if (index < lines.size()) {
                        throw nos::yaml::parse_error(lines[index].number, 1, "unexpected trailing content");
                    }
                    return res;
                }
            };
            ...
            inline void write_node(const nos::trent& tr, std::ostream& os, int indent) {
                switch (tr.get_type()) {
