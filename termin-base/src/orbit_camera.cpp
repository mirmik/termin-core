...
namespace termin {
    namespace {
        ...
        inline float& M(float* m, int col, int row) {
            return m[col * 4 + row];
        }
        ...
        Mat44f look_at_negative_z(const Vec3f& eye, const Vec3f& target, const Vec3f& up) {
        ...
        Mat44f perspective_negative_z(float fov_y, float aspect, float near_clip, float far_clip) {
            Mat44f m;
    ...
    void OrbitCamera::fit_bounds(const Vec3f& bounds_min, const Vec3f& bounds_max) {
    ...
    OrbitCameraRay OrbitCamera::screen_ray(float screen_x, float screen_y, float width, float height) const {
    ...
    std::optional<Vec3f>
    OrbitCamera::world_point_on_z_plane(float screen_x, float screen_y, float width, float height, float z) const {
