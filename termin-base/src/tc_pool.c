...
static void pool_release_arrays(tc_pool_deallocate_fn deallocate,
                                void* user_data,
                                void* data,
                                ...
                                uint32_t* free_list) {
    if (data)
        deallocate(data, user_data);
    if (generations)
        deallocate(generations, user_data);
    if (states)
        deallocate(states, user_data);
    if (free_list)
        deallocate(free_list, user_data);
}

static void
pool_append_free_range(tc_pool* pool, uint32_t* free_list, uint32_t first, uint32_t end, uint32_t* free_count) {
    for (uint32_t i = first; i < end; ++i) {
        const uint32_t index = pool->allocate_low_indices_first ? end - 1u - (i - first) : i;
...
static void pool_advance_generation_epoch(tc_pool* pool) {
    ...
    if (!epoch || pool->capacity == 0 || !pool->generations)
        return;

    uint32_t maximum = pool->generations[0];
    for (uint32_t i = 1; i < pool->capacity; ++i) {
        if (pool->generations[i] > maximum)
            maximum = pool->generations[i];
    }
    if (maximum == UINT32_MAX) {
        epoch->exhausted = true;
        tc_log_error("[%s] handle generation epoch exhausted during shutdown", pool_name(pool));
        return;
    }
...
static void pool_release(tc_pool* pool, bool advance_epoch) {
    if (!pool)
        return;
    if (advance_epoch)
        pool_advance_generation_epoch(pool);

    tc_pool_deallocate_fn deallocate = pool->deallocate ? pool->deallocate : system_deallocate;
    pool_release_arrays(
        deallocate, pool->allocator_user_data, pool->data, pool->generations, pool->states, pool->free_list);
...
static bool pool_grow(tc_pool* pool, uint32_t requested_capacity) {
    ...
    if (new_capacity <= old_capacity || !allocation_size_valid(new_capacity, pool->item_size)) {
        tc_log_error("[%s] invalid pool growth capacity", pool_name(pool));
        return false;
    }

    void* data = pool->allocate((size_t)new_capacity * pool->item_size, pool->allocator_user_data);
    uint32_t* generations =
        (uint32_t*)pool->allocate((size_t)new_capacity * sizeof(uint32_t), pool->allocator_user_data);
    uint8_t* states = (uint8_t*)pool->allocate((size_t)new_capacity * sizeof(uint8_t), pool->allocator_user_data);
    uint32_t* free_list = (uint32_t*)pool->allocate((size_t)new_capacity * sizeof(uint32_t), pool->allocator_user_data);
    if (!data || !generations || !states || !free_list) {
        tc_log_error("[%s] grow allocation failed", pool_name(pool));
        pool_release_arrays(pool->deallocate, pool->allocator_user_data, data, generations, states, free_list);
        return false;
    }

    if (old_capacity != 0) {
        memcpy(data, pool->data, (size_t)old_capacity * pool->item_size);
        memcpy(generations, pool->generations, (size_t)old_capacity * sizeof(uint32_t));
        memcpy(states, pool->states, (size_t)old_capacity * sizeof(uint8_t));
        memcpy(free_list, pool->free_list, (size_t)pool->free_count * sizeof(uint32_t));
    }
    memset((char*)data + (size_t)old_capacity * pool->item_size,
           0,
    ...
    pool_append_free_range(pool, free_list, old_capacity, new_capacity, &free_count);

    pool_release_arrays(
        pool->deallocate, pool->allocator_user_data, pool->data, pool->generations, pool->states, pool->free_list);
    pool->data = data;
...
bool tc_pool_init_rebootstrap(tc_pool* pool,
                              size_t item_size,
...
bool tc_pool_init_ex(tc_pool* pool, size_t item_size, uint32_t initial_capacity, const tc_pool_config* config) {
    if (!pool || item_size == 0)
        return false;

    const uint32_t max_capacity = config && config->max_capacity ? config->max_capacity : UINT32_MAX;
    if (initial_capacity > max_capacity || !allocation_size_valid(initial_capacity, item_size)) {
    ...
    tc_pool_generation_epoch* generation_epoch = config ? config->generation_epoch : NULL;
    ...
    const uint32_t configured_initial_generation = config ? config->initial_generation : 1u;
    ...
    pool->initial_generation = generation_epoch ? generation_epoch->next_generation : configured_initial_generation;
    pool->allocate_low_indices_first = config ? config->allocate_low_indices_first : false;
    pool->name = config ? config->name : "tc_pool";
    pool->allocate = config && config->allocate ? config->allocate : system_allocate;
    pool->deallocate = config && config->deallocate ? config->deallocate : system_deallocate;
    ...
    for (uint32_t i = 0; i < pool->capacity; i++) {
        if (pool->states[i] == TC_SLOT_OCCUPIED) {
            if (pool->generations[i] == UINT32_MAX) {
                ...
                tc_log_error("[%s] retired slot %u after handle generation exhaustion", pool_name(pool), i);
            } else {
                pool->generations[i]++;
...
bool tc_pool_free_slot(tc_pool* pool, tc_handle h) {
    ...
    if (h.index >= pool->capacity)
        return false;
    if (pool->states[h.index] != TC_SLOT_OCCUPIED)
        return false;
    if (pool->generations[h.index] != h.generation)
        return false;

    if (pool->generations[h.index] == UINT32_MAX) {
        pool->states[h.index] = TC_SLOT_RETIRED;
        tc_log_error("[%s] retired slot %u after handle generation exhaustion", pool_name(pool), h.index);
    } else {
...
void* tc_pool_get_checked(const tc_pool* pool, tc_handle h, const char* resource_type) {
    ...
    if (pool && h.index < pool->capacity && pool->generations[h.index] != h.generation) {
        tc_log_error("stale resource handle dereference: type=%s index=%u generation=%u current_generation=%u",
