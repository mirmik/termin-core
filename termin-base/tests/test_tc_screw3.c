...
int main(void)
{
    const tc_screw3 screw =
        tc_screw3_new(TC_VEC3(2.0, 4.0, 6.0), TC_VEC3(8.0, 10.0, 12.0));
    ...
    const tc_pose3 frame =
        tc_pose3_new(tc_quat_identity(), TC_VEC3(3.0, -2.0, 1.0));
    const tc_screw3 twist = tc_screw3_transform_as_twist_by(screw, frame);
    const tc_screw3 recovered_twist =
        tc_screw3_inverse_transform_as_twist_by(twist, frame);
