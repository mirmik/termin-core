...
int main(void) {
    ...
    tc_quat child_rotation = tc_quat_from_axis_angle(
        tc_vec3_unit_z(),
        0.63);
    tc_affine3d parent = tc_affine3d_mul(
        tc_affine3d_translation(5.0, -3.0, 2.0),
        tc_affine3d_scaling(2.0, 0.5, 1.25));
    tc_affine3d child = tc_affine3d_trs(
        TC_VEC3(-1.0, 4.0, 0.75),
        child_rotation,
        TC_VEC3(0.8, 1.4, 2.0));
    tc_vec3 point = TC_VEC3(3.0, -2.0, 1.5);
    tc_vec3 sequential = tc_affine3d_transform_point(
        parent,
        tc_affine3d_transform_point(child, point));
    ...
    double xy_dot = tc_vec3_dot(
        composed_affine.basis.x,
        composed_affine.basis.y);
    GUARD_C_CHECK(fabs(xy_dot) > 1.0e-3);

    tc_affine3d reflection = tc_affine3d_mul(
        tc_affine3d_translation(7.0, -5.0, 3.0),
        tc_affine3d_mul(
            tc_affine3d_rotation(tc_quat_from_euler(0.2, -0.4, 0.7)),
            tc_affine3d_scaling(-2.0, 0.75, 1.5)));
    tc_affine3d inverse = tc_affine3d_identity();
    GUARD_C_CHECK(tc_affine3d_try_inverse(
        reflection,
        1.0e-12,
        &inverse));
    tc_vec3 round_trip = tc_affine3d_transform_point(
        inverse,
        tc_affine3d_transform_point(reflection, point));
    GUARD_C_CHECK(near_vec3(round_trip, point, 1.0e-11));

    tc_affine3d unchanged = tc_affine3d_translation(9.0, 11.0, 13.0);
    GUARD_C_CHECK(!tc_affine3d_try_inverse(
        tc_affine3d_scaling(0.0, 2.0, 3.0),
        1.0e-12,
        &unchanged));
    ...
    GUARD_C_CHECK(tc_affine3d_try_from_matrix4(
        matrix,
        1.0e-12,
        &matrix_round_trip));
    GUARD_C_CHECK(near_vec3(
        tc_affine3d_transform_point(matrix_round_trip, point),
        composed,
        1.0e-12));
