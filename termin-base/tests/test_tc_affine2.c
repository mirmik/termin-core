...
int main(void) {
    ...
    tc_affine2f parent = tc_affine2f_mul(
        tc_affine2f_translation(5.0f, -3.0f),
        tc_affine2f_scaling(2.0f, 0.5f));
    tc_affine2f child = tc_affine2f_mul(
        tc_affine2f_rotation(0.6f),
        tc_affine2f_shear(0.25f, -0.4f));
    tc_vec2f point = TC_VEC2F(3.0f, -2.0f);
    tc_vec2f sequential = tc_affine2f_transform_point(
        parent,
        tc_affine2f_transform_point(child, point));
    tc_vec2f composed = tc_affine2f_transform_point(
        tc_affine2f_mul(parent, child),
        point);
    GUARD_C_CHECK(nearf(sequential.x, composed.x, 1.0e-5f));
    GUARD_C_CHECK(nearf(sequential.y, composed.y, 1.0e-5f));

    tc_affine2f reflection = tc_affine2f_mul(
        tc_affine2f_translation(7.0f, -5.0f),
        tc_affine2f_mul(
            tc_affine2f_rotation(0.37f),
            tc_affine2f_scaling(-2.0f, 0.75f)));
    tc_affine2f inverse = tc_affine2f_identity();
    GUARD_C_CHECK(tc_affine2f_try_inverse(reflection, 1.0e-8f, &inverse));
    tc_vec2f round_trip = tc_affine2f_transform_point(
        inverse,
        tc_affine2f_transform_point(reflection, point));
    ...
    GUARD_C_CHECK(!tc_affine2f_try_inverse(
        tc_affine2f_scaling(0.0f, 2.0f),
        1.0e-8f,
        &unchanged));
    GUARD_C_CHECK(unchanged.tx == 9.0f);
    GUARD_C_CHECK(unchanged.ty == 11.0f);
    GUARD_C_CHECK(!tc_affine2f_try_inverse(
        tc_affine2f_identity(),
        1.0e-8f,
        NULL));
    ...
    GUARD_C_CHECK(nearf(
        pose_point.x,
        (float)(c * point.x - s * point.y + pose.lin.x),
        1.0e-5f));
    GUARD_C_CHECK(nearf(
        pose_point.y,
        (float)(s * point.x + c * point.y + pose.lin.y),
        1.0e-5f));
