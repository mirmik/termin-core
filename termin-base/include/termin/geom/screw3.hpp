...
inline tc_screw3 tc_screw3::velocity_at_origin_from_offset(
    const tc_vec3& offset_from_origin) const
{
    return {ang, lin - ang.cross(offset_from_origin)};
}
...
namespace termin
{
    ...
    static_assert(std::is_same<Screw3, ::tc_screw3>::value,
                  "termin::Screw3 must alias tc_screw3");
    static_assert(std::is_standard_layout<Screw3>::value,
                  "Screw3 must stay ABI-friendly");
    static_assert(std::is_trivially_copyable<Screw3>::value,
                  "Screw3 must stay trivially copyable");
    static_assert(sizeof(Screw3) == sizeof(Vec3) * 2,
                  "Screw3 must stay two Vec3 values");
    static_assert(alignof(Screw3) == alignof(Vec3),
                  "Screw3 alignment must match Vec3");
    static_assert(offsetof(Screw3, ang) == 0, "Screw3.ang offset changed");
    static_assert(offsetof(Screw3, lin) == sizeof(Vec3),
                  "Screw3.lin offset changed");
