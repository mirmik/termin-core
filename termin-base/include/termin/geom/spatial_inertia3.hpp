...
namespace termin {
    ...
    struct SpatialInertia3 {
        ...
        [[nodiscard]] bool is_finite() const noexcept {
            return std::isfinite(mass) && std::isfinite(principal_moments.x) && std::isfinite(principal_moments.y) &&
                   std::isfinite(principal_moments.z) && std::isfinite(inertia_frame.lin.x) &&
                   std::isfinite(inertia_frame.lin.y) && std::isfinite(inertia_frame.lin.z) &&
                   std::isfinite(inertia_frame.ang.x) && std::isfinite(inertia_frame.ang.y) &&
                   std::isfinite(inertia_frame.ang.z) && std::isfinite(inertia_frame.ang.w);
        }

        [[nodiscard]] bool is_valid() const noexcept {
            return is_finite() && mass > 0.0 && principal_moments.x > 0.0 && principal_moments.y > 0.0 &&
                   principal_moments.z > 0.0 && inertia_frame.ang.norm() > 1e-10;
        }
            ...
            for (int row = 0; row < 3; ++row) {
                for (int column = 0; column < 3; ++column) {
                    ...
                    result(column + 3, row + 3) =
                        central(column, row) +
                        mass * ((row == column ? center.dot(center) : 0.0) - center[row] * center[column]);
                }
            }
            return result;
        }
        ...
        [[nodiscard]] Screw3 momentum(Screw3 velocity) const noexcept {
            const Vec3 center = inertia_frame.lin;
            const Vec3 linear_momentum = (velocity.lin + velocity.ang.cross(center)) * mass;
            const Vec3 angular_momentum = central_inertia().transform(velocity.ang) + center.cross(linear_momentum);
        ...
        [[nodiscard]] SpatialInertia3 rotated_by(Quat orientation) const noexcept {
        ...
        [[nodiscard]] SpatialInertia3 transformed_by(const Pose3& pose) const noexcept {
            return {
                mass,
                principal_moments,
        ...
        [[nodiscard]] SpatialInertia3 inverse_transformed_by(const Pose3& pose) const noexcept {
