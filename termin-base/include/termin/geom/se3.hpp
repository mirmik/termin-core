...
namespace termin {
    namespace se3_detail {
        ...
        inline Vec3 left_jacobian_apply(Vec3 rotation_vector, Vec3 value) noexcept {
            ...
            if (angle_squared < 1e-10) {
                ...
                second_coefficient = (angle - std::sin(angle)) / (angle_squared * angle);
            }

            const Vec3 first_cross = rotation_vector.cross(value);
            return value + first_cross * first_coefficient + rotation_vector.cross(first_cross) * second_coefficient;
        }

        inline Vec3 left_jacobian_inverse_apply(Vec3 rotation_vector, Vec3 value) noexcept {
            const double angle_squared = rotation_vector.dot(rotation_vector);
            double coefficient;
            if (angle_squared < 1e-10) {
                ...
                const double angle = std::sqrt(angle_squared);
                coefficient = (1.0 - 0.5 * angle / std::tan(0.5 * angle)) / angle_squared;
            }

            const Vec3 first_cross = rotation_vector.cross(value);
            return value - first_cross * 0.5 + rotation_vector.cross(first_cross) * coefficient;
        }
    } // namespace se3_detail
