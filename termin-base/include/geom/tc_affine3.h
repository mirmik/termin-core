...
#ifdef __cplusplus
#define TC_BASIS3D(x_, y_, z_) tc_basis3d{x_, y_, z_}
#define TC_AFFINE3D(basis_, translation_) tc_affine3d{basis_, translation_}
#else
#define TC_BASIS3D(x_, y_, z_) (tc_basis3d){x_, y_, z_}
#define TC_AFFINE3D(basis_, translation_) (tc_affine3d){basis_, translation_}
#endif
...
static inline tc_vec3 tc_basis3d_transform_vector(
    ...
    tc_vec3 vector) {
    return TC_VEC3(
        basis.x.x * vector.x + basis.y.x * vector.y + basis.z.x * vector.z,
...
static inline tc_basis3d tc_basis3d_mul(
    ...
    tc_basis3d child) {
    return TC_BASIS3D(
        tc_basis3d_transform_vector(parent, child.x),
...
static inline bool tc_basis3d_try_inverse(
    ...
    tc_vec3 row0 = TC_VEC3(
        basis.y.y * basis.z.z - basis.y.z * basis.z.y,
        basis.y.z * basis.z.x - basis.y.x * basis.z.z,
        basis.y.x * basis.z.y - basis.y.y * basis.z.x);
    tc_vec3 row1 = TC_VEC3(
        basis.z.y * basis.x.z - basis.z.z * basis.x.y,
        basis.z.z * basis.x.x - basis.z.x * basis.x.z,
        basis.z.x * basis.x.y - basis.z.y * basis.x.x);
    tc_vec3 row2 = TC_VEC3(
        basis.x.y * basis.y.z - basis.x.z * basis.y.y,
        basis.x.z * basis.y.x - basis.x.x * basis.y.z,
        basis.x.x * basis.y.y - basis.x.y * basis.y.x);

    *out_inverse = TC_BASIS3D(
        TC_VEC3(row0.x * inv_det, row1.x * inv_det, row2.x * inv_det),
...
static inline tc_affine3d tc_affine3d_mul(
    ...
    tc_affine3d child) {
    return TC_AFFINE3D(
        tc_basis3d_mul(parent.basis, child.basis),
        tc_vec3_add(
            parent.translation,
...
static inline bool tc_affine3d_try_inverse(
    ...
    tc_vec3 inverse_translation = tc_basis3d_transform_vector(
        inverse_basis,
        tc_vec3_neg(affine.translation));
...
static inline bool tc_affine3d_try_from_matrix4(
    ...
    double threshold = fabs(epsilon);
    if (fabs(column_major_16[3]) > threshold
        || fabs(column_major_16[7]) > threshold
        || fabs(column_major_16[11]) > threshold
        || fabs(column_major_16[15] - 1.0) > threshold) {
        return false;
    }

    *out_affine = TC_AFFINE3D(
        TC_BASIS3D(
            TC_VEC3(
                column_major_16[0],
                column_major_16[1],
