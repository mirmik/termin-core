...
TC_C_STATIC_INLINE tc_affine2f tc_affine2f_new(float m00, float m01, float m10, float m11, float tx, float ty) {
...
TC_C_STATIC_INLINE tc_affine2f tc_affine2f_trs(tc_vec2f translation, float radians, tc_vec2f scale) {
    float c = cosf(radians);
    float s = sinf(radians);
    return TC_AFFINE2F(c * scale.x, -s * scale.y, s * scale.x, c * scale.y, translation.x, translation.y);
}

TC_C_STATIC_INLINE tc_affine2f tc_affine2f_from_pose2(tc_pose2 pose) {
    float c = (float)cos(pose.ang);
    float s = (float)sin(pose.ang);
    return TC_AFFINE2F(c, -s, s, c, (float)pose.lin.x, (float)pose.lin.y);
}

// parent * child applies child first, then parent.
TC_C_STATIC_INLINE tc_affine2f tc_affine2f_mul(tc_affine2f parent, tc_affine2f child) {
    return TC_AFFINE2F(parent.m00 * child.m00 + parent.m01 * child.m10,
...
TC_C_STATIC_INLINE tc_vec2f tc_affine2f_transform_point(tc_affine2f affine, tc_vec2f point) {
    return TC_VEC2F(affine.m00 * point.x + affine.m01 * point.y + affine.tx,
                    affine.m10 * point.x + affine.m11 * point.y + affine.ty);
}

TC_C_STATIC_INLINE tc_vec2f tc_affine2f_transform_vector(tc_affine2f affine, tc_vec2f vector) {
    return TC_VEC2F(affine.m00 * vector.x + affine.m01 * vector.y, affine.m10 * vector.x + affine.m11 * vector.y);
}
...
TC_C_STATIC_INLINE bool tc_affine2f_is_finite(tc_affine2f affine) {
    return isfinite(affine.m00) && isfinite(affine.m01) && isfinite(affine.m10) && isfinite(affine.m11) &&
           isfinite(affine.tx) && isfinite(affine.ty);
}

TC_C_STATIC_INLINE bool tc_affine2f_try_inverse(tc_affine2f affine, float epsilon, tc_affine2f* out_inverse) {
    ...
    tc_affine2f inverse = TC_AFFINE2F(
        affine.m11 * inv_det, -affine.m01 * inv_det, -affine.m10 * inv_det, affine.m00 * inv_det, 0.0f, 0.0f);
...
TC_C_STATIC_INLINE tc_bounds2f tc_affine2f_transform_bounds(tc_affine2f affine, tc_bounds2f bounds) {
    tc_vec2f p0 = tc_affine2f_transform_point(affine, TC_VEC2F(bounds.x0, bounds.y0));
    tc_vec2f p1 = tc_affine2f_transform_point(affine, TC_VEC2F(bounds.x1, bounds.y0));
    tc_vec2f p2 = tc_affine2f_transform_point(affine, TC_VEC2F(bounds.x0, bounds.y1));
    tc_vec2f p3 = tc_affine2f_transform_point(affine, TC_VEC2F(bounds.x1, bounds.y1));

    return TC_BOUNDS2F(fminf(fminf(p0.x, p1.x), fminf(p2.x, p3.x)),
