...
TC_C_STATIC_INLINE tc_aabb tc_aabb_new(tc_vec3 min_point, tc_vec3 max_point) {
    return TC_AABB(min_point, max_point);
}

TC_C_STATIC_INLINE tc_aabb tc_aabb_zero(void) {
    return TC_AABB(tc_vec3_zero(), tc_vec3_zero());
}

TC_C_STATIC_INLINE void tc_aabb_extend(tc_aabb* box, tc_vec3 point) {
...
TC_C_STATIC_INLINE tc_vec3 tc_aabb_size(tc_aabb box) {
    return tc_vec3_sub(box.max_point, box.min_point);
}

TC_C_STATIC_INLINE tc_vec3 tc_aabb_half_size(tc_aabb box) {
...
TC_C_STATIC_INLINE double tc_aabb_volume(tc_aabb box) {
    tc_vec3 d = tc_aabb_size(box);
    return d.x * d.y * d.z;
}

TC_C_STATIC_INLINE void tc_aabb_get_corners(tc_aabb box, tc_vec3* out_corners) {
...
TC_C_STATIC_INLINE tc_aabb tc_aabb_from_points(const tc_vec3* points, size_t count) {
    if (count == 0) {
        return tc_aabb_zero();
    }
