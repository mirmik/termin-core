...
TC_C_STATIC_INLINE tc_pose2 tc_pose2_new(double ang, tc_vec2 lin) {
    return TC_POSE2(ang, lin);
}

TC_C_STATIC_INLINE tc_pose2 tc_pose2_identity(void) {
    return TC_POSE2(0.0, tc_vec2_zero());
}

TC_C_STATIC_INLINE tc_vec2 tc_pose2_rotate_vector(tc_pose2 p, tc_vec2 v) {
    double c = cos(p.ang);
    double s = sin(p.ang);
    return TC_VEC2(c * v.x - s * v.y, s * v.x + c * v.y);
}

TC_C_STATIC_INLINE tc_vec2 tc_pose2_inverse_rotate_vector(tc_pose2 p, tc_vec2 v) {
    double c = cos(p.ang);
    double s = sin(p.ang);
    return TC_VEC2(c * v.x + s * v.y, -s * v.x + c * v.y);
}

TC_C_STATIC_INLINE tc_vec2 tc_pose2_transform_point(tc_pose2 p, tc_vec2 point) {
    return tc_vec2_add(tc_pose2_rotate_vector(p, point), p.lin);
}

TC_C_STATIC_INLINE tc_vec2 tc_pose2_transform_vector(tc_pose2 p, tc_vec2 vec) {
    return tc_pose2_rotate_vector(p, vec);
}

TC_C_STATIC_INLINE tc_vec2 tc_pose2_inverse_transform_point(tc_pose2 p, tc_vec2 point) {
    return tc_pose2_inverse_rotate_vector(p, tc_vec2_sub(point, p.lin));
}

TC_C_STATIC_INLINE tc_vec2 tc_pose2_inverse_transform_vector(tc_pose2 p, tc_vec2 vec) {
...
TC_C_STATIC_INLINE tc_pose2 tc_pose2_rotation(double angle) {
    return TC_POSE2(angle, tc_vec2_zero());
}

TC_C_STATIC_INLINE tc_pose2 tc_pose2_translation(double x, double y) {
