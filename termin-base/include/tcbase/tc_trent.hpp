...
namespace tc {
    ...
    public:
        ...
        bool is_numer() const noexcept {
            return value_ &&
                   (value_->type == TC_VALUE_INT || value_->type == TC_VALUE_FLOAT || value_->type == TC_VALUE_DOUBLE);
        }
    ...
    public:
        ...
        bool is_numer() const noexcept {
            return view().is_numer();
        }
        bool is_integer() const noexcept {
            return view().is_integer();
        }
        bool is_string() const noexcept {
            return view().is_string();
        }
        bool is_list() const noexcept {
            return view().is_list();
        }
        ...
        bool as_bool(bool fallback = false) const noexcept {
            return view().as_bool(fallback);
        }
        int64_t as_integer(int64_t fallback = 0) const noexcept {
            return view().as_integer(fallback);
        }
        double as_numer(double fallback = 0.0) const noexcept {
            return view().as_numer(fallback);
        }
        const char* as_c_str(const char* fallback = "") const noexcept {
            return view().as_c_str(fallback);
        }
        std::string as_string(std::string fallback = {}) const {
            return view().as_string(std::move(fallback));
        }
        std::size_t size() const noexcept {
            return view().size();
        }
        ...
        void push_back(bool value) noexcept {
            push_back_raw(tc_value_bool(value));
        }
        void push_back(int value) noexcept {
            push_back_raw(tc_value_int(value));
        }
        void push_back(int64_t value) noexcept {
            push_back_raw(tc_value_int(value));
        }
        void push_back(float value) noexcept {
            push_back_raw(tc_value_float(value));
        }
        void push_back(double value) noexcept {
            push_back_raw(tc_value_double(value));
        }
        void push_back(const char* value) noexcept {
            push_back_raw(tc_value_string(value));
        }
        void push_back(const std::string& value) noexcept {
            push_back_raw(tc_value_string(value.c_str()));
        }
    ...
    public:
        ...
        trent& operator=(std::nullptr_t) noexcept {
            reset(tc_value_nil());
            return *this;
        }
        trent& operator=(bool value) noexcept {
            reset(tc_value_bool(value));
            return *this;
        }
        trent& operator=(int value) noexcept {
            reset(tc_value_int(value));
            return *this;
        }
        trent& operator=(int64_t value) noexcept {
            reset(tc_value_int(value));
            return *this;
        }
        trent& operator=(float value) noexcept {
            reset(tc_value_float(value));
            return *this;
        }
        trent& operator=(double value) noexcept {
            reset(tc_value_double(value));
            return *this;
        }
        trent& operator=(const char* value) noexcept {
            reset(tc_value_string(value));
            return *this;
        }
        trent& operator=(const std::string& value) noexcept {
            reset(tc_value_string(value.c_str()));
            return *this;
        }
        ...
        trent_type get_type() const noexcept {
            return view().type();
        }
        trent_type type_value() const noexcept {
            return view().type();
        }
        bool is_nil() const noexcept {
            return view().is_nil();
        ...
        trent_ref list_at(std::size_t index) noexcept {
            return ref().list_at(index);
        }
        trent_view list_at(std::size_t index) const noexcept {
            return view().list_at(index);
        }
        trent_ref operator[](std::size_t index) noexcept {
            return ref()[index];
        }
        ...
        trent_ref::list_ref as_list() noexcept {
            return ref().as_list();
        }
        trent_view::list_view as_list() const noexcept {
            return view().as_list();
        }
        trent_ref::dict_ref as_dict() noexcept {
            return ref().as_dict();
        }
        trent_view::dict_view as_dict() const noexcept {
            return view().as_dict();
