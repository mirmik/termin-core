...
struct tc_ray3
{
    ...
    tc_ray3(const tc_vec3& origin, const tc_vec3& dir) : origin(origin)
    {
        double n = dir.norm();
...
struct tc_quat
{
    ...
    static tc_quat look_rotation(const tc_vec3& forward,
    ...
    static tc_quat slerp(const tc_quat& q1, tc_quat q2, double t)
    {
        ...
        if (dot > 0.9995)
        {
            tc_quat result = {q1.x + t * (q2.x - q1.x),
                              q1.y + t * (q2.y - q1.y),
                              q1.z + t * (q2.z - q1.z),
                              q1.w + t * (q2.w - q1.w)};
...
struct tc_affine2f
{
    ...
    constexpr tc_affine2f(
        float m00, float m01, float m10, float m11, float tx, float ty) noexcept
    ...
    static tc_affine2f
    trs(const tc_vec2f& translation, float radians, const tc_vec2f& scale);
...
struct tc_basis3d
{
    ...
    constexpr tc_basis3d(const tc_vec3& x,
                         const tc_vec3& y,
...
struct tc_affine3d
{
    ...
    constexpr tc_affine3d(const tc_basis3d& basis,
    ...
    static tc_affine3d trs(const tc_vec3& translation,
    ...
    static bool try_from_matrix4(const double* column_major_16,
                                 tc_affine3d& out,
...
struct tc_pose3
{
    ...
    static tc_pose3 looking_at(const tc_vec3& eye,
...
struct tc_general_pose3
{
    ...
    constexpr tc_general_pose3(const tc_quat& ang,
                               const tc_vec3& lin,
                               const tc_vec3& scale = tc_vec3{1.0,
                                                              1.0,
                                                              1.0}) noexcept
    ...
    static tc_general_pose3 looking_at(const tc_vec3& eye,
                                       const tc_vec3& target,
                                       const tc_vec3& up_vec = tc_vec3{
                                           0.0, 0.0, 1.0});
};
...
struct tc_screw3
{
    ...
    tc_screw3
    velocity_at_origin_from_offset(const tc_vec3& offset_from_origin) const;
    tc_screw3 wrench_at_offset(const tc_vec3& offset_from_origin) const;
    tc_screw3
    wrench_at_origin_from_offset(const tc_vec3& offset_from_origin) const;
