...
namespace tc {
    ...
    public:
        ...
        template <typename... Args> static void debug(const std::exception& e, const char* format, Args... args) {
            log_exception_fmt(TC_LOG_DEBUG, e, format, args...);
        }

        template <typename... Args> static void info(const std::exception& e, const char* format, Args... args) {
            log_exception_fmt(TC_LOG_INFO, e, format, args...);
        }

        template <typename... Args> static void warn(const std::exception& e, const char* format, Args... args) {
            log_exception_fmt(TC_LOG_WARN, e, format, args...);
        }

        template <typename... Args> static void error(const std::exception& e, const char* format, Args... args) {
