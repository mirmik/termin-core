...
#define TC_DLIST_HEAD_INIT(name)                                                                                       \
    { &(name), &(name) }
#define TC_DLIST_HEAD(name) tc_dlist_head name = TC_DLIST_HEAD_INIT(name)

#define tc_dlist_entry(ptr, type, member) ((type*)((char*)(ptr) - offsetof(type, member)))
...
#define tc_dlist_for_each(pos, head) for (pos = (head)->next; pos != (head); pos = pos->next)
...
#define tc_dlist_for_each_entry(pos, head, type, member)                                                               \
    for (pos = tc_dlist_entry((head)->next, type, member); &pos->member != (head);                                     \
         pos = tc_dlist_entry(pos->member.next, type, member))

#define tc_dlist_for_each_entry_reverse(pos, head, type, member)                                                       \
    for (pos = tc_dlist_entry((head)->prev, type, member); &pos->member != (head);                                     \
         pos = tc_dlist_entry(pos->member.prev, type, member))

#define tc_dlist_for_each_entry_safe(pos, tmp, head, type, member)                                                     \
    for (pos = tc_dlist_entry((head)->next, type, member), tmp = tc_dlist_entry(pos->member.next, type, member);       \
         &pos->member != (head);                                                                                       \
