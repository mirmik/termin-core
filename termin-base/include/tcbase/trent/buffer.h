...
namespace nos {
    ...
    public:
        ...
        buffer(const std::string_view& str)
            : buffer(str.data(), str.size()) {}

        bool operator==(const buffer& other) const {
            return (_size == other._size) &&
                   (strncmp(_data, other._data, _size < other._size ? _size : other._size) == 0);
        }

        bool operator==(const std::string& other) const {
            return (_size == other.size()) &&
                   (strncmp(_data, other.data(), _size < other.size() ? _size : other.size()) == 0);
        }
