...
namespace nos {
    template <class Key, class T> class flat_map {
        ...
        using const_reverse_iterator = typename std::vector<pair>::const_reverse_iterator;
    ...
    public:
        ...
        reference operator[](const Key& key) {
            auto it = std::find_if(storage.begin(), storage.end(), [&key](const pair& p) { return p.first == key; });
        ...
        iterator find(const Key& key) {
            return std::find_if(storage.begin(), storage.end(), [&key](const pair& p) { return p.first == key; });
        }
        ...
        size_type count(const Key& key) const {
            return std::count_if(storage.begin(), storage.end(), [&key](const pair& p) { return p.first == key; });
        }

        template <class... Args> std::pair<iterator, bool> emplace(Key key, Args&&... args) {
            auto it = std::find_if(storage.begin(), storage.end(), [&key](const pair& p) { return p.first == key; });
            if (it != storage.end()) {
