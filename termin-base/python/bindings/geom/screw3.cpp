...
namespace termin {
    ...
    void bind_screw3(nb::module_& m) {
        nb::class_<Screw3>(m, "Screw3")
            ...
            .def("inverse_transform_as_twist_by", &Screw3::inverse_transform_as_twist_by)
            .def("transform_as_wrench_by", &Screw3::transform_as_wrench_by)
            .def("inverse_transform_as_wrench_by", &Screw3::inverse_transform_as_wrench_by)
            .def("velocity_at_offset", &Screw3::velocity_at_offset)
            .def("velocity_at_origin_from_offset", &Screw3::velocity_at_origin_from_offset)
            .def("wrench_at_offset", &Screw3::wrench_at_offset)
            .def("wrench_at_origin_from_offset", &Screw3::wrench_at_origin_from_offset)
            .def("scaled", &Screw3::scaled)
            .def("copy", [](const Screw3& s) { return s; })
            .def("to_vector_vw_order",
                 [](const Screw3& s) { return nb::make_tuple(s.lin.x, s.lin.y, s.lin.z, s.ang.x, s.ang.y, s.ang.z); })
            .def("to_vector_wv_order",
                 [](const Screw3& s) { return nb::make_tuple(s.ang.x, s.ang.y, s.ang.z, s.lin.x, s.lin.y, s.lin.z); })
            .def("to_vw_array",
                 [](const Screw3& s) { return nb::make_tuple(s.lin.x, s.lin.y, s.lin.z, s.ang.x, s.ang.y, s.ang.z); })
            .def("to_wv_array",
                 [](const Screw3& s) { return nb::make_tuple(s.ang.x, s.ang.y, s.ang.z, s.lin.x, s.lin.y, s.lin.z); })
            ...
            .def("__repr__", [](const Screw3& s) {
                return "Screw3(ang=Vec3(" + std::to_string(s.ang.x) + ", " + std::to_string(s.ang.y) + ", " +
                       std::to_string(s.ang.z) + "), lin=Vec3(" + std::to_string(s.lin.x) + ", " +
