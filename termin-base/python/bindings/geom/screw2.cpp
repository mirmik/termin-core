...
namespace termin {
    ...
    void bind_screw2(nb::module_& m) {
        nb::class_<Screw2>(m, "Screw2")
            ...
            .def_prop_rw(
                "lin", [](const Screw2& s) { return s.lin; }, [](Screw2& s, const Vec2& lin) { s.lin = lin; })
            .def(nb::self + nb::self)
            .def(nb::self - nb::self)
            .def(nb::self * double())
            .def(double() * nb::self)
            ...
            .def("kinematic_carry", [](const Screw2& self, const Vec2& arm) { return self.kinematic_carry(arm); })
            .def("force_carry", [](const Screw2& self, const Vec2& arm) { return self.force_carry(arm); })
            .def("twist_carry", [](const Screw2& self, const Vec2& arm) { return self.twist_carry(arm); })
            .def("wrench_carry", [](const Screw2& self, const Vec2& arm) { return self.wrench_carry(arm); })
            ...
            .def("to_vector_vw_order", [](const Screw2& s) { return nb::make_tuple(s.lin.x, s.lin.y, s.ang); })
            .def("to_vector_wv_order", [](const Screw2& s) { return nb::make_tuple(s.ang, s.lin.x, s.lin.y); })
            ...
            .def("__repr__", [](const Screw2& s) {
                return "Screw2(ang=" + std::to_string(s.ang) + ", lin=Vec2(" + std::to_string(s.lin.x) + ", " +
