...
namespace {
    ...
    public:
        ...
        bool enabled() const {
            return tc_profiler_enabled();
        }
        void set_enabled(bool v) {
            tc_profiler_set_enabled(v);
        }

        void begin_frame() {
            tc_profiler_begin_frame();
        }
        void begin_frame_with_info(double start_time_ms,
                                   double interval_ms,
                                   double target_interval_ms,
                                   double deadline_lateness_ms,
                                   int missed_intervals) {
        ...
        void begin_section_muted(const std::string& name) {
            tc_profiler_begin_section_muted(name.c_str());
        }
        void end_section() {
        ...
        int frame_count() const {
            return tc_profiler_frame_count();
        }

        int history_count() const {
            return tc_profiler_history_count();
        }
...
void bind_profiler(nb::module_& m) {
    ...
    nb::class_<TcProfiler>(m, "TcProfiler")
        ...
        .def("begin_frame_with_info",
             &TcProfiler::begin_frame_with_info,
             nb::arg("start_time_ms"),
             nb::arg("interval_ms"),
             nb::arg("target_interval_ms"),
             nb::arg("deadline_lateness_ms"),
