...
namespace termin::python_host {
    ...
    namespace {
        ...
        InitResult status_result(const Config& config, std::string_view operation, PyStatus status) {
            InitResult result;
        ...
        RuntimeAbi query_runtime_abi() {
            ...
            PyObject* gil_value = PyObject_CallFunction(get_config_var, "s", "Py_GIL_DISABLED");
        ...
        InitResult validate_initialized_runtime(const Config& config) {
            try {
                ...
                if (expected.version != actual.version || expected.soabi != actual.soabi ||
                    expected.free_threaded != actual.free_threaded) {
                    return InitResult{
                        .ok = false,
                        .exit_requested = false,
                        .exit_code = 1,
                        .error = config.host_name + ": Python ABI mismatch: expected " + expected.describe() +
                                 ", initialized " + actual.describe(),
                    };
                }
                ...
                return InitResult{
                    ...
                    .exit_requested = false,
                    .exit_code = 1,
                    .error = config.host_name + ": Python ABI validation failed: " + error.what() +
    ...
    std::string RuntimeAbi::describe() const {
        ...
        output << version << "|" << soabi << "|free_threaded=" << (free_threaded ? "true" : "false");
        return output.str();
    }

    RuntimeAbi expected_abi() {
        RuntimeAbi identity;
        identity.version = std::to_string(PY_MAJOR_VERSION) + "." + std::to_string(PY_MINOR_VERSION);
    ...
    InitResult initialize(const Config& config) {
        ...
        if (!config.home.empty()) {
            ...
            status = PyConfig_SetString(&python_config, &python_config.home, home.c_str());
#else
            status = PyConfig_SetBytesString(&python_config, &python_config.home, config.home.string().c_str());
#endif
        ...
        status = PyConfig_SetBytesString(&python_config, &python_config.program_name, arguments.front().c_str());
        if (PyStatus_Exception(status)) {
        ...
        status = PyConfig_SetBytesArgv(&python_config, static_cast<Py_ssize_t>(argv.size()), argv.data());
        if (PyStatus_Exception(status)) {
