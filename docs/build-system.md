...
Schema-v1 документ и его persistence API принадлежат toolkit-neutral модулю
...
JSON; CLI backend использует ту же модель и не содержит собственного JSON
парсера. Импорт profile store не загружает argparse и target build wrappers,
поэтому эту модель можно напрямую использовать в editor UI.
...
- `shader_targets`: для desktop profile обязательный ordered list из
  `vulkan`, `opengl`, `d3d11`.
...
Для desktop profile `shader_targets` одновременно задаёт набор поставляемых
