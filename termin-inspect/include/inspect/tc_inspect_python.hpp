...
namespace tc {
    ...
    inline tc_value nb_to_tc_value(nb::object obj) {
        ...
        if (nb::isinstance<nb::dict>(obj)) {
            ...
            for (auto item : nb::cast<nb::dict>(obj)) {
                ...
                tc_value_dict_set(&dict, key.c_str(), nb_to_tc_value(nb::borrow<nb::object>(item.second)));
            }
            return dict;
        }
    ...
    inline nb::object tc_value_to_nb(const tc_value* v) {
        ...
        case TC_VALUE_DICT: {
            ...
            for (size_t i = 0; i < v->data.dict.count; i++) {
                d[nb::str(v->data.dict.entries[i].key)] = tc_value_to_nb(v->data.dict.entries[i].value);
            }
            return d;
        }
    ...
    public:
        ...
        static InspectFacetBuilder build_python_fields(const std::string& type_name, nb::dict fields_dict);

        // Field access with nb::object (uses tc_value internally)
        static nb::object
        get(InspectRegistry& reg, void* obj, const std::string& type_name, const std::string& field_path);
        static void set(InspectRegistry& reg,
                        void* obj,
        ...
        static void deserialize_all_py(InspectRegistry& reg,
                                       void* obj,
                                       const std::string& type_name,
                                       const nb::dict& data,
                                       void* context = nullptr);
        ...
        static void deserialize_component_fields_over_python(InspectRegistry& reg,
                                                             void* ptr,
                                                             nb::object obj,
                                                             const std::string& type_name,
                                                             const nb::dict& data,
    ...
    inline InspectFacetBuilder build_python_inspect_facet(const std::string& type_name, nb::dict fields_dict) {
        return InspectRegistryPythonExt::build_python_fields(type_name, std::move(fields_dict));
    }

    inline nb::object
    InspectRegistry_get(InspectRegistry& reg, void* obj, const std::string& type_name, const std::string& field_path) {
        return InspectRegistryPythonExt::get(reg, obj, type_name, field_path);
    }

    inline void InspectRegistry_set(InspectRegistry& reg,
                                    void* obj,
                                    const std::string& type_name,
                                    const std::string& field_path,
                                    nb::object value,
    ...
    inline void InspectRegistry_deserialize_all_py(
        InspectRegistry& reg, void* obj, const std::string& type_name, const nb::dict& data, void* context = nullptr) {
        InspectRegistryPythonExt::deserialize_all_py(reg, obj, type_name, data, context);
    }

    inline void InspectRegistry_deserialize_component_fields_over_python(InspectRegistry& reg,
                                                                         void* ptr,
                                                                         nb::object obj,
                                                                         const std::string& type_name,
                                                                         const nb::dict& data,
