...
namespace tc {
    ...
    template <typename H> tc_value serialize_handle_value(const H& h) {
        if constexpr (requires(const H& x) { x.serialize_to_value(); }) {
            ...
            static_assert(always_false_v<H>,
                          "Handle type must provide serialize_to_value() or {is_valid, uuid, name}.");
        }
    }

    template <typename H> H deserialize_handle_value(const tc_value* v, void* context) {
    ...
    template <typename H> bool handle_value_is_valid(const H& value) {
    ...
    template <typename H> std::optional<H> deserialize_handle_value_checked(const tc_value* value, void* context) {
        if (value == nullptr || value->type != TC_VALUE_DICT)
            return std::nullopt;
        tc_value* uuid = tc_value_dict_get(const_cast<tc_value*>(value), "uuid");
        if (uuid == nullptr)
            return H{}; // Canonical null handle.
        if (uuid->type != TC_VALUE_STRING || uuid->data.s == nullptr)
            return std::nullopt;
        if (uuid->data.s[0] == '\0')
            return H{};
        H result = deserialize_handle_value<H>(value, context);
        if (!handle_value_is_valid(result))
            return std::nullopt;
        return result;
    }
        ...
        registry.register_kind(
            ...
            [](const tc_value* v, void* context) -> std::any {
                ...
                for (size_t i = 0; i < v->data.list.count; i++) {
                    auto value = deserialize_handle_value_checked<H>(&v->data.list.items[i], context);
                    if (!value)
                        return std::any();
