...
namespace tc {
    ...
    inline InspectFieldSpec inspect_field_spec(const char* type_name,
                                               const char* path,
                                               const char* label,
                                               const char* kind,
                                               double min = -1e9,
    ...
    inline InspectFieldSpec inspect_accessor_field_spec(const char* type_name,
                                                        const char* path,
                                                        const char* label,
                                                        const char* kind,
                                                        bool is_serializable = true,
...
#define TC_INSPECT_API __attribute__((visibility("default")))
    ...
    class TC_INSPECT_API InspectFacetBuilder {
        ...
        bool validate_field(const InspectFieldInfo& info) {
            ...
            if (!info.type_name.empty() && info.type_name != type_name()) {
                return reject("field '" + info.path + "' belongs to type '" + info.type_name + "', expected '" +
                              type_name() + "'");
            }
            if (info.kind == "button") {
                if (!info.action) {
                    return reject("button field '" + type_name() + "." + info.path + "' has no action");
                }
            } else if (!info.getter && !info.setter) {
                return reject("field '" + type_name() + "." + info.path + "' has no callbacks");
            } else if (info.is_serializable && !info.getter) {
                return reject("serializable field '" + type_name() + "." + info.path + "' has no getter");
            }
            return true;
        }
    ...
    public:
        ...
        const InspectFieldInfo* find_field(const std::string& requested_type, const std::string& path) const {
        ...
        bool set_metadata_key(const std::string& key, const tc_value* value) {
            ...
            tc_value_dict_set(&_payload->metadata, key.c_str(), value ? tc_value_copy(value) : tc_value_nil());
            return true;
        }

        bool add_field(InspectFieldInfo info) {
            if (!valid() || !validate_field(info))
                return false;
            for (const InspectFieldInfo& existing : _payload->fields) {
                if (existing.path == info.path) {
                    return reject("duplicate field path '" + type_name() + "." + info.path + "'");
                }
            }
        ...
        template <typename C, typename T> bool add(T C::*member, const InspectFieldSpec& spec) {
            ...
            info.getter = [member, kind, type, path](void* obj) -> tc_value {
                ...
                return KindRegistryCpp::instance().serialize_field(kind, std::any(value), type, path);
            };
            info.setter = [member, kind, type, path](void* obj, tc_value value, void* context) -> bool {
        ...
        bool add(const char* type_name,
                 T C::*member,
                 const char* path,
                 const char* label,
                 const char* kind,
                 RangeArgs&&... range_args) {
            return add<C, T>(member,
                             inspect_field_spec(type_name, path, label, kind, std::forward<RangeArgs>(range_args)...));
        }

        template <typename C, typename T, typename GetterFn, typename SetterFn>
        bool add_with_callbacks(const InspectFieldSpec& spec, GetterFn getter_fn, SetterFn setter_fn) {
            return add_with_callbacks<C, T>(spec.type_name,
                                            spec.path,
                                            spec.label,
                                            spec.kind,
                                            std::move(getter_fn),
        ...
        bool add_with_callbacks(const char* type_name,
                                const char* path,
                                const char* label,
                                const char* kind,
                                GetterFn getter_fn,
            ...
            info.getter = [getter_fn = std::move(getter_fn), kind_copy, type_copy, path_copy](void* obj) {
                ...
                return KindRegistryCpp::instance().serialize_field(kind_copy, std::any(value), type_copy, path_copy);
            };
        ...
        bool add_with_accessors(const char* type_name,
                                const char* path,
                                const char* label,
                                const char* kind,
                                GetterFn getter_fn,
                                SetterFn setter_fn,
                                FlagArgs&&... flag_args) {
            InspectFieldSpec spec =
                inspect_accessor_field_spec(type_name, path, label, kind, std::forward<FlagArgs>(flag_args)...);
            InspectFieldInfo info = make_inspect_field_info(spec);
            const std::string kind_copy = info.kind;
            const std::string type_copy = this->type_name();
            const std::string path_copy = info.path;
            ...
            info.setter = [setter_fn = std::move(setter_fn),
                           kind_copy](void* obj, tc_value value, void* context) -> bool {
                std::any decoded = KindRegistryCpp::instance().deserialize(kind_copy, &value, context);
                if (!decoded.has_value())
                    return false;
                try {
        ...
        bool add_serializable(const char* path, GetterFn getter_fn, SetterFn setter_fn) {
            InspectFieldInfo info;
            info.path = path;
            info.label = path;
            ...
            info.setter = [setter_fn = std::move(setter_fn)](void* obj, tc_value value, void*) -> bool {
        ...
        template <typename C, typename T, typename GetterFn, typename SetterFn>
        bool add_with_accessor_choices(const char* type_name,
                                       const char* path,
                                       const char* label,
                                       const char* kind,
                                       GetterFn getter_fn,
            ...
            InspectFieldInfo info = make_inspect_field_info(inspect_field_spec(type_name, path, label, kind));
        ...
        bool add_button(std::string path, std::string label, std::function<void(void*, const InspectContext&)> action) {
            InspectFieldInfo info;
            info.type_name = type_name();
        ...
        bool attach_to(tc_runtime_type_descriptor* descriptor) {
            ...
            return tc_runtime_type_descriptor_add_facet(
                descriptor, TC_RUNTIME_TYPE_FACET_INSPECT_FIELDS, payload, detail::destroy_inspect_facet, nullptr, 1);
        }
    ...
    class TC_INSPECT_API InspectRegistry {
        ...
        InspectFacetPayload* inspect_facet(const std::string& type_name) const {
            return static_cast<InspectFacetPayload*>(
                tc_runtime_type_registry_get_facet(type_name.c_str(), TC_RUNTIME_TYPE_FACET_INSPECT_FIELDS));
        }
    ...
    public:
        ...
        bool has_type(const std::string& type_name) const {
            return tc_runtime_type_registry_has_facet(type_name.c_str(), TC_RUNTIME_TYPE_FACET_INSPECT_FIELDS);
        }
        ...
        tc_value type_metadata(const std::string& type_name) const {
            ...
            if (!parent.empty()) {
                ...
                const InspectFacetPayload* payload = inspect_facet(type_name);
                if (payload && payload->has_metadata && payload->metadata.type == TC_VALUE_DICT &&
                    result.type == TC_VALUE_DICT) {
        ...
        bool set_tc_value(
            void* obj, const std::string& type_name, const std::string& field_path, tc_value value, void* context) {
            const InspectFieldInfo* f = find_field(type_name, field_path);
            if (!f) {
        ...
        void deserialize_all(void* obj, const std::string& type_name, const tc_value* data, void* context = nullptr) {
            ...
            for (const auto& f : all_fields(type_name)) {
                ...
                if (!field_val || field_val->type == TC_VALUE_NIL)
                    continue;
    ...
    bool stage_inspect_field(InspectFacetBuilder& builder, T C::*member, const InspectFieldSpec& spec) {
        return builder.add<C, T>(member, spec);
    }

    template <typename C, typename T, typename... RangeArgs>
    bool stage_inspect_field(InspectFacetBuilder& builder,
                             T C::*member,
                             const char* type_name,
                             const char* path,
                             const char* label,
        ...
        return stage_inspect_field<C, T>(
            builder, member, inspect_field_spec(type_name, path, label, kind, std::forward<RangeArgs>(range_args)...));
    }

    template <typename C, typename T>
    bool stage_inspect_field_choices(InspectFacetBuilder& builder,
                                     T C::*member,
                                     const char* type_name,
                                     const char* path,
                                     const char* label,
        ...
        info.getter = [member, kind_copy, type_copy, path_copy](void* obj) -> tc_value {
            ...
            return KindRegistryCpp::instance().serialize_field(kind_copy, std::any(val), type_copy, path_copy);
        };
        info.setter = [member, kind_copy, type_copy, path_copy](void* obj, tc_value value, void* context) -> bool {
            if constexpr (std::is_same_v<std::decay_t<T>, std::string>) {
                if (kind_copy == "enum") {
    ...
    template <typename C>
    bool stage_inspect_button_method(InspectFacetBuilder& builder,
                                     const char* path,
                                     const char* label,
                                     void (C::*method)()) {
...
#define INSPECT_FIELD_ACCESSORS(cls, type, name, label, kind, getter_fn, setter_fn, ...)                               \
    static void _register_inspect_##name(::tc::InspectFacetBuilder& builder) {                                         \
        (void)builder.add_with_accessors<cls, type>(#cls, #name, label, kind, getter_fn, setter_fn, ##__VA_ARGS__);    \
    }
...
#define SERIALIZABLE_FIELD(cls, name, getter_expr, setter_expr)                                                        \
    inline void register_serialize_##cls##_##name(::tc::InspectFacetBuilder& builder) {                                \
        ...
        info.getter = [](void* obj) -> tc_value {                                                                      \
            return static_cast<cls*>(obj)->getter_expr;                                                                \
        };                                                                                                             \
