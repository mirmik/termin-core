...
namespace tc {
    ...
    static bool python_inspect_get_field(const char* type_name, size_t index, tc_field_info* out, void* ctx) {
        (void)ctx;
        const InspectFieldInfo* info =
            InspectRegistry::instance().get_field_by_index(type_name ? type_name : "", index);
        if (!info)
            return false;
    ...
    static bool python_inspect_find_field(const char* type_name, const char* path, tc_field_info* out, void* ctx) {
        (void)ctx;
        const InspectFieldInfo* info =
            InspectRegistry::instance().find_field(type_name ? type_name : "", path ? path : "");
        if (!info)
            return false;
        info->fill_c_info(out);
        return true;
    ...
    static tc_value python_inspect_get(void* obj, const char* type_name, const char* path, void* ctx) {
        ...
        try {
            ...
            tc::Log::error(e,
                           "[Inspect] Python get failed for type '%s' field '%s'",
                           type_name ? type_name : "",
                           path ? path : "");
        }
        return tc_value_nil();
    ...
    static bool
    python_inspect_set(void* obj, const char* type_name, const char* path, tc_value value, void* context, void* ctx) {
        (void)ctx;
        nb::gil_scoped_acquire gil;
        try {
            ...
            if (!applied) {
                tc_log(TC_LOG_ERROR,
                       "[Inspect] Python setter rejected value for type '%s' field '%s'",
                       type_name ? type_name : "",
                       path ? path : "");
            }
            return applied;
            ...
            tc::Log::error(e,
                           "[Inspect] Python set failed for type '%s' field '%s'",
                           type_name ? type_name : "",
                           path ? path : "");
        }
        return false;
    ...
    static void python_inspect_action(void* obj, const char* type_name, const char* path, void* ctx) {
        ...
        try {
            ...
            InspectRegistry::instance().action_field(
                obj, type_name ? type_name : "", path ? path : "", inspect_context);
        } catch (const std::exception& e) {
            tc::Log::error(e,
                           "[Inspect] Python action failed for type '%s' field '%s'",
                           type_name ? type_name : "",
                           path ? path : "");
        }
    }
    ...
    void init_python_inspect_vtable() {
        static tc_inspect_lang_vtable python_vtable = {python_inspect_has_type,
    ...
    InspectFacetBuilder InspectRegistryPythonExt::build_python_fields(const std::string& type_name,
                                                                      nb::dict fields_dict) {
    ...
    nb::object InspectRegistryPythonExt::get(InspectRegistry& reg,
                                             void* obj,
    ...
    void InspectRegistryPythonExt::set(InspectRegistry& reg,
                                       void* obj,
                                       const std::string& type_name,
                                       const std::string& field_path,
                                       nb::object value,
    ...
    void InspectRegistryPythonExt::deserialize_all_py(
        InspectRegistry& reg, void* obj, const std::string& type_name, const nb::dict& data, void* context) {
    ...
    void InspectRegistryPythonExt::deserialize_component_fields_over_python(InspectRegistry& reg,
                                                                            void* ptr,
                                                                            nb::object obj,
                                                                            const std::string& type_name,
                                                                            const nb::dict& data,
