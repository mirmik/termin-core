...
void MeshRenderer::register_type() {
    ...
    descriptor.inspect().add<MeshRenderer, bool>(
        &MeshRenderer::cast_shadow,
        InspectFieldSpec{"MeshRenderer", "cast_shadow", "Cast Shadow", "bool"});
    if (!descriptor.commit()) {
        throw std::runtime_error("failed to register MeshRenderer");
    }
}
```

Фабрика компонента, parent, metadata, requirements, capabilities и inspect
